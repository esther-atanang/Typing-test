...
const TypingArea = ({ passage, onInputClick, onReset, onSound }: Props) => {
    ...
    const handleTypingAreaClick = () => {
        // If the test hasn't started, start it
        if (!progress.begin) {
            handleStart();
            return;
        }
    ...
    return (
        <div
            ...
            className='relative mx-4 mt-6 flex min-h-0 flex-1 flex-col text-xl leading-8 sm:mx-8 sm:text-2xl sm:leading-9 md:mx-32 md:text-4xl md:leading-12'
        >
            {/* Hidden input that receives focus and opens the mobile keyboard */}
            <input
                ref={inputRef}
                type='text'
                ...
                >
                    ...
                    >
                        {passage.match(/\S+|\s+/g)!.map((token, j) => {
                            ...
                            if (token === " ") {
                                const lightUp =
                                    (arrayOfWords.length > j && arrayOfWords[j] !== " ")
                                ...
                                return (
                                    <span
                                        ...
                                        className={`whitespace-pre ${
                                            ((progress.wordCount % 2 !== 0) &&
                                                progress.wordCount === j) &&
                                            "underline text-neutral-500"
                                        } ${lightUp}`}
                                    >
                                        &nbsp;
                                    </span>
                            ...
                            return (
                                <span key={j} className="inline-flex">
                                    {[...token].map((char, i) => {
                                        ...
                                        const currentWord =
                                            j < arrayOfWords.length
                                                ? arrayOfWords[j]
                                                : "";

                                        // This lights up characters typed in the current word.
                                        const lightUp =
                                            currentWord &&
                                        ...
                                        const letterIndex = getLetterIndex(
                                            results.totalTypedWords,
                                            progress.prevWordLength
                                        );
            ...
            >
                <Button
                    ...
                    className="text-sm sm:text-base md:text-lg p-6 sm:px-5 sm:py-4 md:p-6"
                >
